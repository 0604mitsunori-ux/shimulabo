# -*- coding: utf-8 -*-
"""index.html へ SEO新規20本のカード＋ランキング＋本数(591)を挿入（1回限り）。"""
import os, io, sys
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
IDX = os.path.join(ROOT, 'index.html')

import gen_sims_seo5
GRAD = {
  ...
  '健康・カラダ': 'linear-gradient(135deg,#fef2f2,#fee2e2)',
  'クルマ・乗り物': 'linear-gradient(135deg,#eff6ff,#e0e7ff)',
  '人生・自分ごと': 'linear-gradient(135deg,#fefce8,#fef9c3)',
  '冠婚葬祭・贈り物': 'linear-gradient(135deg,#fdf4ff,#fae8ff)',
  '子ども・育児': 'linear-gradient(135deg,#fffbeb,#fef3c7)',
}
SLUG = {
  'マネー・保険・不動産':'finance','お金・時間':'money','税金・確定申告':'tax','仕事・働き方':'work',
  '健康・カラダ':'health','クルマ・乗り物':'car','人生・自分ごと':'life','冠婚葬祭・贈り物':'manner','子ども・育児':'kids',
}

sims = gen_sims_seo5.SIMS
assert len(sims) == 20, len(sims)

with io.open(IDX, encoding='utf-8') as f:
    html = f.read()
assert 'sims/kotei-shisanzei/index.html' not in html, '既に挿入済み（中止）'
...
html = html.replace('<b>571</b>本 公開中', '<b>591</b>本 公開中', 1)

with io.open(IDX, 'w', encoding='utf-8') as f:
    f.write(html)
print(f'patched index.html: +{len(sims)} cards/ranking, count=591')
