# -*- coding: utf-8 -*-
"""index.html の data-cat="kidedu" シミュ全てに、スマイルゼミCTAを挿入（冪等）。"""
import os, io, re, sys
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from smile_cta import CTA_HTML
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
with io.open(os.path.join(ROOT,'index.html'), encoding='utf-8') as f: idx = f.read()
ids = list(dict.fromkeys(re.findall(r'data-cat="kidedu" href="sims/([a-z0-9-]+)/', idx)))
print('kidedu sims:', len(ids))
MARKER='<!-- smile-cta -->'; ANCHOR='  <section class="req-banner">'; ins=0
for sid in ids:
