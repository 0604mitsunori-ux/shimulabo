# -*- coding: utf-8 -*-
"""index.html の data-cat="uranai" シミュ全てに、電話占いウラナCTAを挿入（冪等）。"""
import os, io, re, sys
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from urana_cta import CTA_HTML
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
IDX = os.path.join(ROOT, 'index.html')
with io.open(IDX, encoding='utf-8') as f: idx = f.read()
ids = list(dict.fromkeys(re.findall(r'data-cat="uranai" href="sims/([a-z0-9-]+)/', idx)))
print('uranai sims:', len(ids))
MARKER='<!-- urana-cta -->'; ANCHOR='  <section class="req-banner">'; ins=0
for sid in ids:
