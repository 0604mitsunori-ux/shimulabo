# -*- coding: utf-8 -*-
"""index.html へ 「家系・ルーツ」追加10本のカード＋ランキング＋本数(691→701)を挿入（1回限り）。カテゴリは配線済み。"""
import os, io, sys
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
IDX = os.path.join(ROOT, 'index.html')

import gen_sims_kakei2
GRAD = 'linear-gradient(135deg,#f0fdf4,#dcfce7)'
SLUG = 'kakei'

sims = gen_sims_kakei2.SIMS
assert len(sims) == 10, len(sims)

with io.open(IDX, encoding='utf-8') as f:
    html = f.read()
assert 'sims/kibiki/index.html' not in html, '既に挿入済み（中止）'
...
html = html.replace('<b>691</b>本 公開中', '<b>701</b>本 公開中', 1)

with io.open(IDX, 'w', encoding='utf-8') as f:
    f.write(html)
print(f'patched index.html: +{len(sims)} cards/ranking, count=701')
